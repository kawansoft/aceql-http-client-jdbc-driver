...
public class AceQLHttpApi {
	...
	public AceQLHttpApi(String serverUrl, String database, String username, char[] password, Proxy proxy,
		...
		try {
			...
			String url = serverUrl + "/database/" + database + "/username/" + username + "/connect" + "?password="
	...
	private String callWithGet(String url)
		...
		InputStream in = null;
		try {
			in = callWithGetReturnStream(url);
	...
	private InputStream callWithPost(String action, Map<String, String> parameters)
			throws MalformedURLException, IOException, ProtocolException, UnsupportedEncodingException {
	 ...
	 *             if any Exception occurs
	 */
	public int executeUpdate(String sql, boolean isPreparedStatement, Map<String, String> statementParameters)
			throws AceQLException {
		...
		try {
			...
			try {
				in = callWithPost(action, parametersMap);
				if (in != null) {
					ByteArrayOutputStream out = new ByteArrayOutputStream();
					IOUtils.copy(in, out);
	 ...
	 *             if any Exception occurs
	 */
	public InputStream executeQuery(String sql, boolean isPreparedStatement, Map<String, String> statementParameters)
			throws AceQLException {
		...
		try {
			...
			//parametersMap.put("prepared_statement", new Boolean(isPreparedStatement).toString());
			//parametersMap.put("gzip_result", new Boolean(gzipResult).toString());
			//parametersMap.put("pretty_printing", new Boolean(prettyPrinting).toString());
