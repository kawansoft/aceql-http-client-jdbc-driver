...
public class AceQLApiHttpExample {
	...
	public static void main(String[] args) throws Exception {
		...
		AceQLHttpApi aceQLHttpApi = new AceQLHttpApi(serverUrl, database, username, password.toCharArray());
		...
		System.out.println("aceQLHttpApi.getServerVersion(): " + aceQLHttpApi.getServerVersion());
		System.out.println("aceQLHttpApi.getClientVersion(): " + aceQLHttpApi.getClientVersion());

		aceQLHttpApi.setAutoCommit(true);

		System.out.println("aceQlApi.getAutoCommit() : " + aceQLHttpApi.getAutoCommit());
		System.out.println("aceQlApi.isReadOnly()    : " + aceQLHttpApi.isReadOnly());
		System.out.println("aceQlApi.getHoldability(): " + aceQLHttpApi.getHoldability());
		System.out.println("aceQlApi.getTransactionIsolation() : " + aceQLHttpApi.getTransactionIsolation());
		...
		if (doInsert) {
			...
			for (int i = 1; i < 10; i++) {
				...
				builder.setParameter(j++, AceQLTypes.VARCHAR, "John_" + customerId);
				builder.setParameter(j++, AceQLTypes.VARCHAR, "Smith_" + customerId);
				builder.setParameter(j++, AceQLTypes.VARCHAR, customerId + " César Avenue");
				builder.setParameter(j++, AceQLTypes.VARCHAR, "Town_" + customerId);
				builder.setParameter(j++, AceQLTypes.VARCHAR, customerId + "");
				builder.setParameter(j++, AceQLTypes.VARCHAR, customerId + "-12345678");

				aceQLHttpApi.executeUpdate(sql, true, builder.getStatementParameters());
		...
		if (doSelectPrepStatement) {
			...
			InputStream input = aceQLHttpApi.executeQuery(sql, true, statementParameters);
			printResultSet(input);
...
//			builder.setParameter(j++, AceQLTypes.INTEGER, new Integer(customerId).toString());
//			builder.setParameter(j++, AceQLTypes.INTEGER, new Integer(itemId).toString());
//			builder.setParameter(j++, AceQLTypes.VARCHAR, "description_" + itemId);
//			builder.setParameter(j++, AceQLTypes.NUMERIC, new Integer(itemId * 1000).toString());
//			builder.setParameter(j++, AceQLTypes.DATE, new Long(System.currentTimeMillis()).toString());
//			builder.setParameter(j++, AceQLTypes.TIMESTAMP, new Long(System.currentTimeMillis()).toString());
//			builder.setParameter(j++, AceQLTypes.BLOB, blobId);
//			builder.setParameter(j++, AceQLTypes.NUMERIC, new Integer(1).toString());
//			builder.setParameter(j++, AceQLTypes.INTEGER, new Integer(itemId * 1000).toString());

			builder.setParameter(j++, AceQLTypes.INTEGER, "" + customerId);
			builder.setParameter(j++, AceQLTypes.INTEGER, "" + itemId);
			builder.setParameter(j++, AceQLTypes.VARCHAR, "description_" + itemId);
			builder.setParameter(j++, AceQLTypes.NUMERIC, "" + (itemId * 1000));
			builder.setParameter(j++, AceQLTypes.DATE, "" + System.currentTimeMillis());
			builder.setParameter(j++, AceQLTypes.TIMESTAMP, "" + System.currentTimeMillis());
			builder.setParameter(j++, AceQLTypes.BLOB, blobId);
			...
			if (doBlobDownload) {
				...
				if (blobId != null) {
					...
					InputStream in = null;
					try {
						in = aceQLHttpApi.blobDownload(blobId);
	...
	private static String extractBlobId(String jsonResultSet) throws IOException {
