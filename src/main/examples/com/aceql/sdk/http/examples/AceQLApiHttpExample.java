...
public class AceQLApiHttpExample {
	...
	public static void main(String[] args) throws Exception {
		...
		AceQLHttpApi aceQLHttpApi = new AceQLHttpApi(serverUrl, database, username, password.toCharArray(), null, null, null);
		...
		if (doInsert) {
			...
			for (int i = 1; i < 10; i++) {
				...
				builder.setInParameter(j++, AceQLTypes.VARCHAR, "John_" + customerId);
				builder.setInParameter(j++, AceQLTypes.VARCHAR, "Smith_" + customerId);
				builder.setInParameter(j++, AceQLTypes.VARCHAR, customerId + " César Avenue");
				builder.setInParameter(j++, AceQLTypes.VARCHAR, "Town_" + customerId);
				builder.setInParameter(j++, AceQLTypes.VARCHAR, customerId + "");
				builder.setInParameter(j++, AceQLTypes.VARCHAR, customerId + "-12345678");

				aceQLHttpApi.executeUpdate(sql, true, false, builder.getHttpFormattedStatementParameters(), null);
		...
		if (doSelectPrepStatement) {
			...
			InputStream input = aceQLHttpApi.executeQuery(sql, true, false, statementParameters);
			printResultSet(input);
		...
		if (doBlobUpload) {
			...
			builder.setInParameter(j++, AceQLTypes.VARCHAR, "description_" + itemId);
			builder.setInParameter(j++, AceQLTypes.NUMERIC, "" + (itemId * 1000));
			builder.setInParameter(j++, AceQLTypes.DATE, "" + System.currentTimeMillis());
			builder.setInParameter(j++, AceQLTypes.TIMESTAMP, "" + System.currentTimeMillis());
