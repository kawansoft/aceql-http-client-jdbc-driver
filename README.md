...
```java
    ...
    Connection connection = DriverManager.getConnection(url, info);
```
The request headers will be intercepted on the server side by the `validate` method of your Java class that implements the [RequestHeadersAuthenticator](https://www.aceql.com/rest/soft/8.0/javadoc/org/kawanfw/sql/api/server/auth/headers/RequestHeadersAuthenticator.html) interface.

See also the Headers Authentication sub-section in the [aceql-server.properties](https://www.aceql.com/rest/soft/8.0/src/aceql-server.properties) file for details about declaring your `RequestHeadersAuthenticator` concrete implementation on server side.
